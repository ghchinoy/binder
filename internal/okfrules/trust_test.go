...
func fmFrom(pairs ...any) *okf.OrderedMap {
	m := NewOrderedMap()
...
func TestTrustTier(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			c := &okf.Concept{Trust: ProjectTrust(tc.fm, "Metric")}
...
func TestIsStale(t *testing.T) {
	...
	for _, tc := range cases {
		c := &okf.Concept{Trust: okf.TrustSignals{StaleAfter: tc.staleAfter}}
...
func TestValidateTrustIsAdvisoryOnly(t *testing.T) {
	...
	c := &okf.Concept{ID: "c", Type: AttestedComputationType, Frontmatter: fm, Trust: ProjectTrust(fm, AttestedComputationType)}
	findings := ValidateTrust(c, okf.DefaultSpecVersion)
	if len(findings) == 0 {
...
func TestValidateTrustCleanConcept(t *testing.T) {
	...
	c := &okf.Concept{ID: "c", Type: "Note", Frontmatter: fm, Trust: ProjectTrust(fm, "Note")}
	if findings := ValidateTrust(c, okf.DefaultSpecVersion); len(findings) != 0 {
...
func TestValidateTrustShapeAdvisories(t *testing.T) {
	...
	c := &okf.Concept{ID: "c", Type: "Metric", Frontmatter: fm, Trust: ProjectTrust(fm, "Metric")}
	findings := ValidateTrust(c, okf.DefaultSpecVersion)
...
func TestIsProtectedTrustKey(t *testing.T) {
	// IsProtectedTrustKey agrees with the authoritative okf.ProtectedTrustKeys()
	// slice for every protected key, and never protects a refreshable lifecycle key.
