...
func CodeRegions(body string) []okf.Span {
	src := []byte(body)
	doc := mdParser.Parse(text.NewReader(src))
	var spans []okf.Span
	_ = ast.Walk(doc, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
...
func InCodeRegion(offset int, spans []okf.Span) bool {
	for _, s := range spans {
...
func linesSpan(n ast.Node) (okf.Span, bool) {
	lines := n.Lines()
	if lines == nil || lines.Len() == 0 {
		return okf.Span{}, false
	}
	first := lines.At(0)
	last := lines.At(lines.Len() - 1)
	return okf.Span{Start: first.Start, End: last.Stop}, true
}

// textSpan returns the byte range spanning an inline node's text segments.
func textSpan(n ast.Node) (okf.Span, bool) {
	...
	return okf.Span{Start: start, End: end}, true
}
