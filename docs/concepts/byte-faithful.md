...
## Byte-faithful round-trip, on a recognised fence
...
advisory). A file with **no** recognisable fence at all is still read as plain
and synthesized over, leaving its content in the body as text, with exit `0`,
nothing skipped, and no warning. For the bounds known to remain even on a
...
## Never fabricate trust, on a recognised fence
...
flags, frontmatter on a recognised fence that needed no read-boundary
...
advisory), not a silent round-trip. On a file with **no** recognisable fence at
all this does not hold: the file is treated as plain, so its content is left in
the body as text while binder synthesizes keys of its own, among them a `type`, a
`title`, and a `generated` provenance stamp, with exit `0`, nothing skipped, and
no warning. `generated` is a key binder itself protects: `--overwrite-keys
generated` is refused as a trust-provenance key.
