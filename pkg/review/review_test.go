...
func TestReviewCountsTypesTiersOrphansUnresolved(t *testing.T) {
	...
	calc := concept("calc", okfrules.AttestedComputationType, "Calc")
	...
	if r.Tiers[okfrules.TierHumanReviewed] != 1 {
		t.Errorf("human-reviewed = %d, want 1", r.Tiers[okfrules.TierHumanReviewed])
	}
	if r.Tiers[okfrules.TierMachineConfirmed] != 1 {
		t.Errorf("machine-confirmed = %d, want 1", r.Tiers[okfrules.TierMachineConfirmed])
	}
	if r.Tiers[okfrules.TierUnverified] != 2 {
		t.Errorf("unverified = %d, want 2 (intro, calc)", r.Tiers[okfrules.TierUnverified])
	}
...
func TestReviewDetectsRecoveredFrontmatterBody(t *testing.T) {
	...
	// okfrules.MarkRecovered by hand, so it never puts a genuinely-unparseable file on
	...
	okfrules.MarkRecovered(recovered.Frontmatter, "unparseable-frontmatter")
	// UNTERMINATED fence (recovered as body) — same marker, surfaced uniformly.
	unterm := concept("unterm", "Note", "Unterm")
	unterm.Body = "---\ntitle: never closed\ntags: [a, b]\n\n# Heading After\n\nStill body.\n"
	okfrules.MarkRecovered(unterm.Frontmatter, "unparseable-frontmatter")
...
func TestReviewDiscloseUnparsedFromBundleField(t *testing.T) {
	...
	rec := concept("bad", "", "")
	rec.Frontmatter = okfrules.NewOrderedMap() // deliberately unmarked
	b := &okf.Bundle{
		Root:     "/b",
...
func TestReviewUnparsedDedupesMarkerAndField(t *testing.T) {
	rec := concept("bad", "", "")
	okfrules.MarkRecovered(rec.Frontmatter, "unparseable-frontmatter")
	b := &okf.Bundle{
		Root:     "/b",
		Concepts: []*okf.Concept{rec},
		Unparsed: []okf.UnparsedConcept{{ID: "bad", RelPath: "bad.md", Err: "invalid frontmatter"}},
