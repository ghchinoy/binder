...
// It implements okf.Codec and okfrules.LinkGraph and is the default codec, selected
...
// Codec implements okf.Codec and okfrules.LinkGraph.
type Codec struct{}
...
var (
	...
	_ okfrules.LinkGraph = (*Codec)(nil)
)
...
func (c *Codec) ParseConcept(relPath string, raw []byte) (*okf.Concept, error) {
	...
	typ := okfrules.AsString(mapGet(fm, "type"))
	con := &okf.Concept{
		ID:                  id,
		RelPath:             relPath,
		Type:                typ,
	...
	con.Trust = okfrules.ProjectTrust(fm, typ)
	return con, nil
}
...
func (c *Codec) ExtractLinks(fromConceptID, body string) []okf.Link {
	raw := okfrules.ExtractMarkdownLinks(body)
...
func parseFrontmatter(fmText string) (*okf.OrderedMap, error) {
	if strings.TrimSpace(fmText) == "" {
		return okfrules.NewOrderedMap(), nil
	}
