...
// Span is a half-open [Start,End) byte range within a body. It is part of the
// published okf vocabulary (MUST seam): the code-region helpers that produce
// spans live in the internal okfrules package, but the value type they return
// crosses into okf.Bundle-adjacent surfaces, so the type itself is public.
type Span struct {
	Start int
	End   int
}
