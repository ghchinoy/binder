...
import (
	...
	"github.com/ghchinoy/binder/pkg/validate"
)
...
func tierOfBytes(t *testing.T, raw []byte) okfrules.Tier {
	t.Helper()
	c, err := native.New().ParseConcept("a.md", raw)
	if err != nil {
		t.Fatalf("re-parsing converted output failed: %v\n%q", err, raw)
	}
	c.Trust = okfrules.ProjectTrust(c.Frontmatter, "")
	return okfrules.TrustTier(c)
}
...
func TestConvertAC1_BOM_PreservesVerifiedAndDiscloses(t *testing.T) {
	...
	if got := tierOfBytes(t, got); got != okfrules.TierHumanReviewed {
...
func TestConvertAC3_RecoverAsBodyFiresAfterNormalization(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			res, err := validate.Bundle(out, native.New(), okf.DefaultSpecVersion)
			if err != nil {
				t.Fatal(err)
			}
			if !res.Conformant() {
