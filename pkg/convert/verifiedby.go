...
func applyVerifiedBy(c *okf.Concept, opts Options) VerifiedResult {
	...
	default:
		...
		return VerifiedResult{Advisory: fmt.Sprintf("verified: value %q is not a {by,at} stamp or list of them (spec §5.2); preserved unchanged, no verified stamp appended", okfrules.AsString(v))}
	}
	...
	if !opts.VerifiedByExplicit {
		for _, e := range list {
			if m, ok := e.(map[string]any); ok {
				if by := okfrules.AsString(m["by"]); by != "" && by != opts.VerifiedBy {
	...
	for _, e := range list {
		if m, ok := e.(map[string]any); ok {
			if okfrules.AsString(m["by"]) == opts.VerifiedBy && okfrules.AsString(m["at"]) == at {
				return VerifiedResult{}
			}
		}
	}
