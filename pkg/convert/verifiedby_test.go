...
func TestVerifiedByAppends(t *testing.T) {
	...
	if okfrules.AsString(m["by"]) != "human:ghchinoy" {
		t.Errorf("by = %v, want human:ghchinoy", m["by"])
	}
	if okfrules.AsString(m["at"]) != "2023-11-14T22:13:20Z" {
...
func TestVerifiedByResidualASkipsNonExplicitCoSign(t *testing.T) {
	...
	m := list[0].(map[string]any)
	if okfrules.AsString(m["by"]) != "human:ahormati" || okfrules.AsString(m["at"]) != "2020-01-01T00:00:00Z" {
...
func TestVerifiedByTierDerived(t *testing.T) {
	...
	human.Trust = okfrules.ProjectTrust(human.Frontmatter, human.Type)
	if tier := okfrules.TrustTier(human); tier != okfrules.TierHumanReviewed {
	...
	bot.Trust = okfrules.ProjectTrust(bot.Frontmatter, bot.Type)
	if tier := okfrules.TrustTier(bot); tier != okfrules.TierMachineConfirmed {
