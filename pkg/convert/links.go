...
//
// Matches are found over okfrules.MaskCode(body), which blanks the CONTENT of every
...
func (r *linkResolver) rewrite(body, srcRel string) (string, []okf.Link) {
	...
	scan := okfrules.MaskCode(body) // offset-preserving; scan prose, slice from body
