...
func mapTrust(c *okf.Concept, opts Options) {
	...
	for _, key := range opts.SourceKeys {
		...
		val := strings.TrimSpace(okfrules.AsString(v))
		if val == "" {
			continue
		}
...
func sourceKey(e any) string {
	...
	if !ok {
		return okfrules.AsString(e)
	}
	return okfrules.AsString(m["resource"]) + "\x00" + okfrules.AsString(m["title"]) + "\x00" + okfrules.AsString(m["author"])
}

func hasNonEmpty(fm *okf.OrderedMap, key string) bool {
	v, ok := fm.Get(key)
	return ok && strings.TrimSpace(okfrules.AsString(v)) != ""
}
