...
type Report struct {
	...
	Stale            []string  `json:"stale"`             // okfrules.IsStale as of today
...
func Lint(concepts []*okf.Concept, facts []convert.SourceFacts, today string, entrypoints []string) *Report {
	...
	// the single pinned convention (okfrules.HeadingSlugs). Precomputed once so a
	...
	for _, c := range concepts {
		...
		for _, s := range okfrules.HeadingSlugs(c.Body) {
			set[s] = true
		}
		slugSets[c.ID] = set
	}
	...
	for _, c := range concepts {
		...
		for _, ml := range okfrules.ExtractMarkdownLinks(c.Body) {
	...
	// Check 4 — stale: okfrules.IsStale as of today. today is deterministic (the
