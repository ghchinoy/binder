...
// ddl.go and introduces NO new trust logic: is_human reuses okfrules.IsHumanActor —
// the SAME predicate okfrules.TrustTier applies — and the verified[] attestations are
// copied VERBATIM from the concept (okfrules.ProjectTrust / projectActorstamps), never
...
//     by test against okfrules.TrustTier / okfrules.IsStale).
package graph
...
// primary key; is_human is okfrules.IsHumanActor(by), the SAME predicate TrustTier
...
// okfrules.IsHumanActor. A node with no verified[] contributes no rows. There is no
	...
	for _, n := range p.Nodes {
		for i, v := range n.Verified {
			rows = append(rows, NodeVerifiedRow{
				...
				By:      v.By,
				At:      v.At,
				IsHuman: okfrules.IsHumanActor(v.By),
			})
		}
	}
	return rows
...
//
// The CASE mirrors okfrules.TrustTier exactly: a node with no NodeVerified rows
...
// mirrors okfrules.IsStale exactly: a node without stale_after is never stale;
...
// Proven by test to equal okfrules.TrustTier for the same concept, so the frozen
// Nodes.tier is exactly reconstructible from the child table (never a stored or
// fabricated trust fact).
func RecomputeTier(rows []NodeVerifiedRow) okfrules.Tier {
	if len(rows) == 0 {
		return okfrules.TierUnverified
	}
	for _, r := range rows {
		if r.IsHuman {
			return okfrules.TierHumanReviewed
		}
	}
	return okfrules.TierMachineConfirmed
}
...
// date. Proven by test to equal okfrules.IsStale. A node without stale_after is never
