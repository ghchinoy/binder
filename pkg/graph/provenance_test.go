...
// is_human = okfrules.IsHumanActor. The expected rows are written out explicitly (not
...
func TestNodeVerifiedRowsByteFaithful(t *testing.T) {
	...
	// is_human must equal the SHARED predicate okfrules.IsHumanActor for every row —
	// never a second definition of "human".
	for _, r := range got {
		if r.IsHuman != okfrules.IsHumanActor(r.By) {
			t.Errorf("row %q seq %d: is_human=%v, IsHumanActor=%v", r.NodeKey, r.Seq, r.IsHuman, okfrules.IsHumanActor(r.By))
		}
	}
}
...
// okfrules.TrustTier / okfrules.IsStale exactly, for several chosen dates. RecomputeTier /
...
func TestDerivationRecompute(t *testing.T) {
	...
	for _, pn := range p.Nodes {
		...
		if got, want := RecomputeTier(rowsByKey[pn.Key]), okfrules.TrustTier(c); got != want {
		...
		for _, d := range dates {
			...
			if got, want := RecomputeStale(staleAfterByKey[pn.Key], d), okfrules.IsStale(c, d); got != want {
...
// disagree with okfrules.TrustTier for the machine-confirmed node, proving the
...
func TestDerivationRecomputeControl(t *testing.T) {
	...
	wrongTier := func(rows []NodeVerifiedRow) okfrules.Tier {
		if len(rows) == 0 {
	...
	want := okfrules.TrustTier(conceptByKey["node-gen"])
	if got == want {
