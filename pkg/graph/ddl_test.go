...
func TestProjectTierStaleSnapshot(t *testing.T) {
	...
	if got := byKey["node-alpha"]; got.Tier != okfrules.TierHumanReviewed || !got.Stale || got.StaleAfter != "2020-01-01" {
		t.Errorf("alpha = %+v; want human-reviewed, stale, stale_after 2020-01-01", got)
	}
	if got := byKey["node-beta"]; got.Tier != okfrules.TierMachineConfirmed || got.Stale || got.StaleAfter != "" {
		t.Errorf("beta = %+v; want machine-confirmed, not stale, no stale_after", got)
	}
	if got := byKey["node-gamma"]; got.Tier != okfrules.TierUnverified || got.Stale || got.StaleAfter != "2099-01-01" {
