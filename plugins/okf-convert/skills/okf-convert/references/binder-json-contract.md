...
was captured from real `binder/0.5.3` output. The `internal/plugindocs` drift
...
```jsonc
{
  ...
  "colon_space_scalars": []                    // a frontmatter key whose UNQUOTED plain
                                               // scalar contains ": " (e.g. `title: A: B`),
                                               // which YAML reads as a nested mapping;
                                               // advisory only — never gates, not even
                                               // under --strict
}
```
...
```jsonc
{
  "binder": "binder/0.5.3", "command": "config", "schema": "binder.config/v1",
  "result": {
