...
So the question was asked of every scanner here, and each now has a floor:
  ...
  3. command discovery    -> REQUIRED_COMMANDS (identity) and
     MIN_DISCOVERED_COMMANDS (a backstop against partial collapse).
...
# commands are the ones the must-reach inventory already drives, so they are not
# a magic number — if discovery cannot see `convert`, the thing doing the seeing
# is broken, whatever the count says. The count is a backstop for a PARTIAL
# collapse (the root block parsing but recursion into subcommands failing), and
# is a MINIMUM well under today's 15 so that removing a command is not an
# inventory edit.
REQUIRED_COMMANDS = {"convert", "enrich", "config"}
MIN_DISCOVERED_COMMANDS = 8
...
def main() -> int:
    ...
    top_level = {p[0] for p in discovered if p}
    missing_named = sorted(REQUIRED_COMMANDS - top_level)
    if missing_named:
        coverage_fail.append(
            ("sweep:discovery",
             "command discovery did not find " + ", ".join(missing_named)
             + " (Cobra's `Available Commands:` header may have been reworded, "
               "collapsing the breadth sweep)",
             len(top_level), len(REQUIRED_COMMANDS), "required command(s)")
        )
