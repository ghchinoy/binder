...
// verbs as MCP tools whose payloads are the SAME binder.report/v1 envelopes as
// `--json`. The tools registered are convert, validate, review, lint, graph,
// list_graphs, and query_graph (see internal/mcp; the authoritative set is what
// newServer registers). It is a transport, not a report-producing command, so
// it has no `--json` flag (design §Non-Goals).
//
...
func newMCPCmd(codec okf.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "mcp",
		Short: "Run binder as a stdio MCP server (additive verbs as MCP tools)",
